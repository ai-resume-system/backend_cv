...
@Injectable()
export class GetAdminUserGrowthQuery extends BaseUsecase {
  constructor(
    @Inject('IUserRepository')
    private readonly userRepository: IUserRepository,
    private readonly redis: RedisAdapter,
  ) {
    super(new Logger(GetAdminUserGrowthQuery.name));
  }

  async execute(dto: IRequestAdminGrowthDto): Promise<IResponseApiAdminGrowthDto> {
    return this.runSafe('[Get Admin User Growth]:', async () => {
      const resolvedWindow = resolveAnalyticsWindow(dto);
      const version = await this.redis.getVersion(
        CACHE_VERSION_KEYS.ADMIN_ANALYTICS_USER_GROWTH,
      );
      const cacheKey = `${CACHE_KEYS.ADMIN_ANALYTICS_USER_GROWTH}:v${version}:${stableHash(resolvedWindow)}`;
      const cached =
      ...
      const rows = await this.userRepository.getUserGrowthSeries(
